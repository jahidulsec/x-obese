...
const createNewSteps = async (info: createStepsInputsTypes) => {
  const data = await prisma.steps.create({
    data: {
      ...info,
    },
  });
...
const updateOne = async (
  ...
  const updatedData = await prisma.workout.update({
    where: { id: id },
    data: { ...info },
  });
  return updatedData;
...
const getActivityHistory = async (
  ...
  const pointsMap: Record<
    string,
    { steps: number; calories: number; workoutTime: number }
  > = {};
  ...
  sortedLabels.forEach((label) => {
    ...
    result.charts.workoutTime.push({
      label,
      value: pointsMap[label].workoutTime / (60 * 1000),
    }); // in minutes
  });
